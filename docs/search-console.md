...
1. Add the `mingxu.jackmeds.top` Domain property in Google Search Console and complete DNS verification.
2. Submit `https://mingxu.jackmeds.top/sitemap.xml` once. The sitemap is generated during every web build and contains the stable tool, Agent, guide, bilingual application, about and privacy URLs (23 URLs at the current release).
