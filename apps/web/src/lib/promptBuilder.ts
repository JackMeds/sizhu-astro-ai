...
function formatBaziFacts(profile: AstroProfile) {
  ...
  return profile.bazi.facts.natal
    .map((fact) => {
...
function formatZiweiDetails(profile: AstroProfile) {
  ...
  const mutagens = profile.ziwei.natalMutagens?.length
    ...
    : "未取到";
  const palaces = profile.ziwei.palaces
    .slice(0, 12)
    .map((palace, index) => {
...
export function buildChartDataBlock(profile: AstroProfile) {
  ...
  return [
    `基础资料：${profile.input.name}，${profile.input.gender === "male" ? "男命" : "女命"}，历法${profile.input.calendar === "solar" ? "阳历/公历" : "农历/阴历"}，正式时间口径${profile.time.effective.label}，标准时${profile.time.standard.isoLocal}，有效时${profile.time.effective.isoLocal}，出生地${profile.input.location?.name || "未填"}，经度${profile.input.location?.longitude ?? "未填"}。`,
...
export function buildPrompt(profile: AstroProfile, mode: PromptMode, format: PromptFormat) {
  ...
  const sections = [
    ...
    `五行分布：${elements}。旺衰边界提示：${profile.bazi.strengthHint}`,
    `确定性关系事实：\n${formatBaziFacts(profile)}`,
    `大运摘要：${luck || "暂无大运数据"}。`,
    `紫微结构化摘要：\n${formatZiweiDetails(profile)}`,
    `数据提醒：${warnings}。`,
    `输出要求：先列程序计算事实，再说明采用的传统规则与条件，最后给综合解释；不要把候选合化或有争议规则写成既定事实。`,
    `请输出：1. 关键结论；2. 计算事实；3. 传统规则及适用条件；4. 当前大运/流年切入点；5. 紫微交叉观察；6. 分歧与待确认项。`
  ];
