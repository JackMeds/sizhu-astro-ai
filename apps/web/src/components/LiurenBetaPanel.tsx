...
export function LiurenBetaPanel(){
  ...
  return <section className="liuren-beta liuren-complete panel" aria-labelledby="liuren-title">
    ...
    {castingMethod==="branch"?<label><span>指定占时</span><select value={castingBranch} onChange={e=>setCastingBranch(e.target.value)}>{branches.map((b,i)=><option key={b} value={b}>{i+1}. {b}时</option>)}</select></label>:null}
    ...
    <AnimatePresence mode="wait">{chart?<motion.div className="liuren-beta-result" key={`${chart.calendar.dayGanZhi}-${chart.calendar.hourGanZhi}`} initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:.38,ease:[.22,1,.36,1]}}>
      ...
      <div className="liuren-section-title"><span>四课</span><small>上下神 · 天将 · 生克关系</small></div><div className="liuren-courses">{chart.complete.fourLessons.map(c=><article key={c.name}><small>{c.name}</small><strong>{c.upper}{c.lower}</strong><span>{c.god}</span><em>{c.relation}</em></article>)}</div>
      <div className="liuren-section-title"><span>三传</span><small>{chart.complete.transmissionPattern} · {chart.complete.transmissionRule}</small></div><div className="liuren-transmissions">{chart.complete.threeTransmissions.map(i=><article key={i.stage}><div><small>{i.stage}</small>{i.isVoid?<i>空</i>:null}</div><strong>{i.branch}</strong><b>{i.god}</b><p><span>{i.liuQing||"—"}</span><span>{i.dunGan?`遁${i.dunGan}`:"遁空"}</span><span>{i.wuxing}{i.seasonState?`·${i.seasonState}`:""}</span></p><em>{i.dayRelation||i.relation}</em></article>)}</div>
