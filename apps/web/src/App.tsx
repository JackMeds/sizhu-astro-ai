...
export function App() {
  ...
  function acceptProfile(nextProfile: AstroProfile, actor: WorkspaceActor, label?: string) {
    ...
    dispatch({
      ...
      profile: nextProfile,
      actor,
      label: label ?? (actor === "agent" ? "Agent created a birth chart" : "You created a birth chart")
    });
    setHistory((current) => {
  ...
  function commitProfile(nextForm: FormState) {
    acceptProfile(buildProfile(nextForm), "user");
  }
  ...
  function generate() {
    ...
    if (form.trueSolarTime !== "none" && !form.longitude) {
      setFormError("选择太阳时校正时，请填写出生地经度。");
      return;
    }
    try {
      commitProfile(form);
    } catch (error) {
      setFormError(error instanceof Error ? error.message : "排盘失败，请检查输入。");
    }
  }
  ...
  function tryExample() {
    try {
      setForm(exampleForm);
      commitProfile(exampleForm);
    } catch (error) {
      setFormError(error instanceof Error ? error.message : "示例命盘生成失败。");
    }
  }
  ...
  function selectHistory(item: HistoryItem) {
    ...
    acceptProfile(item.profile, "user", "You restored a chart from local history");
  }
            ...
            <aside className="side-stack">
              {profile && !inputExpanded ? (
                <motion.section animate={{ opacity: 1, y: 0 }} className="panel input-summary-panel profile-summary-v2" initial={{ opacity: 0, y: 8 }}>
                  ...
                  <small>{profile.input.timezone} · {profile.time.effective.label} → {profile.time.effective.shichen}时</small>
